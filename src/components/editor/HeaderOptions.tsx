...
import { getFilesDetails, updateFile } from "@/lib/supabase/queries";
...
const HeaderOption = () => {
  ...
  const updateIconId = async (iconId: string) => {
    ...
    dispatch({type:"UPDATE_FILE",payload:{file:updateData,fileId,workspaceId,folderId}})
    try {
      const { error } = await updateFile(updateData, fileId);
      if (error) throw error;
  ...
  useEffect(() => {
    ...
    const fetchFileDetails = async () => {
      ...
      setBanner(data[0]?.bannerUrl || null);
    };
      ...
      <div className="w-full min-h-[20vh] group/parent flex flex-col items-center justify-end">
        ...
        {banner && (
          <div className="relative w-full h-fit group mb-8">
            {/* image */}
            <DragImagePosition fileId={fileDetails.id} isDragable={isDragable} url={banner} onChangePosition={setPosition}/>
              ...
              <span
                onClick={() =>{
                  if(isDragable){
                    ...
                    window.localStorage.setItem(fileDetails.id,JSON.stringify(position))
                  }
        ...
        <Options
          ...
          banner={banner}
        />
        <div onDoubleClick={handleDoubleClick} className="my-4 flex justify-start w-4/6">
          <input onBlur={handleTitleBlur} value={tempTitle ?? fileDetails.title} className="text-3xl select-none font-semibold tracking-wide focus:outline-none bg-transparent" readOnly={!isTitleEditing}
  ...
  return (
    <div className="w-4/6 flex items-start relative">
      <div className="w-fit gap-2 grid grid-cols-3 group/options transition-all transform duration-300 ease-in">
        {/* Icon */}
        <div className={cn("relative", { "col-span-full": emoji !== "" ,"absolute -top-16":emoji.length>0})}>
          <EmojiPicker getValue={(s)=>{
            setEmoji(s)
            updateIcon(s)
            }}>

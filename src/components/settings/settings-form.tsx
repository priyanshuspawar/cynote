...
const SettingsForm = () => {
  ...
  const { selectedEntities, workspaceApi } = useAppSelector((state) => state);
  const [error, setError] = useState<{
    type: "workspace-name";
    error: string;
  } | null>(null);
  const { workspaceId, selectedWorkspace } = selectedEntities;
  //WIP PAYMENT PORTAL
  ...
  const workspaceNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!workspaceId) return;
    if (error && error.type === "workspace-name" && e.target.value) {
      setError(null);
    }
    //wip
  ...
  const onChangeWorkspaceLogo = async (
    ...
    if (!error) {
      ...
      await updateWorkspace({ workspace: { logo: data.path }, workspaceId });
  ...
  useEffect(() => {
    ...
    setWorkspaceDetails(selectedWorkspace);

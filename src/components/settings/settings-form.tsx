...
const SettingsForm = () => {
    ...
    const workspaceNameChange = async(e:React.ChangeEvent<HTMLInputElement>)=>{
        if(!workspaceId || !e.target.value)return;
        dispatch({
            type:'UPDATE_WORKSPACE',payload:{workspaceId,workspace:{title:e.target.value}}
        })
        if(titleTimerRef.current) clearTimeout(titleTimerRef.current);
        titleTimerRef.current = setTimeout(async () => {
            const {data,error} = await updateWorkspace({ title: e.target.value }, workspaceId);
            if(error){
                toast({variant:"destructive",title:"Error",description:"Error while updating workspace"})
